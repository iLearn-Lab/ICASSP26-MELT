...
def test_cirr_submit_result(model, testset, save_dir, name, batch_size = 16):
    ...
    for i, img_data in enumerate(tqdm(candidate_img)):
        ...
        if len(imgs) >= batch_size or i == len(candidate_img) - 1:
            ...
            imgs = []
    # candidate_features = torch.vstack(candidate_features) 
    candidate_features = np.concatenate(candidate_features) # (N,D)
    sims = np.matmul(all_queries, candidate_features) #all_queries.dot(all_imgs.T)
    sims = sims.squeeze()
    sims = sims.max(-1)
...
if __name__ == '__main__':
    ...
    #_, preprocess = clip.load('ViT-B/16', device='cpu', jit=False, download_root="/data/lizixu/czw/PreModel")
    ...
    # model, vis_processors, _ = load_model_and_preprocess(name=blip_model_name, model_type=backbone,
    ...
    #from transformers import CLIPImageProcessor#laionCLIP-ViT-B-32-laion2B-s34B-b79K
    #preprocess = CLIPImageProcessor.from_pretrained("/root/laionCLIP-ViT-B-32-laion2B-s34B-b79K", torch_dtype=torch.float16, local_files_only=True)
    testset = datasets.CIRR(path="/root/autodl-tmp/data/CIR_data/cirr_data/CIRR/",transform=preprocess)
    import sys

    model_dir = sys.argv[1]  # "/root/autodl-tmp/Sinic-Net/cirr_recon_0.5target_0.25compose_QTT_feature_BatchFirst_P4-Q8-mu0.05-nu3-kappa-0.3-tau0.1_Head16_laionCLIP_bs64_twodata/"
    import os
    ...
    for i in file_ls:
        if ".pt" in i and f'CIRR_pred_ranks_recall{i[:-3]}.json' not in file_ls:
            ...
            print(i[:-3] + " start")
            test_cirr_submit_result(model, save_dir=model_dir, testset=testset, batch_size=16, name=i[:-3])
            print(i[:-3] + " end")
